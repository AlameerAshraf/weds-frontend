...
export class ChecklistGridComponent implements OnInit {
  ...
  getAllCheckLists() {
    ...
    const getAllItemsURL = `${urls.GET_ALL_CHECKLISTS_ADMIN}/${constants.APP_IDENTITY_FOR_ADMINS}`;
  ...
  editEntity(id: any) {
    ...
    const targetTheme = this.checkLists.find(x => x._id == id);
  ...
  deleteEntity(id: any) {
    ...
    const deleteURL = `${urls.DELETE_CHECKLIST_ADMIN}/${constants.APP_IDENTITY_FOR_ADMINS}/${id}`;
  ...
  loadScripts() {
    const scripts = ['assets/scripts/custom.js'];
