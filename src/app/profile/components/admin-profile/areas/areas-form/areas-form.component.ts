...
export class AreasFormComponent implements OnInit, AfterViewInit {
  ...
  createNewEntity() {
    ...
    this.http
      .Post(createURL, {}, { area: this.area })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "Area has been saved succesfully",
            "Area has been updated, Bingo!"
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/areas-defaults`
          );
        } else {
          this.ngxSpinner.hide();
          this.toastr.error(
            "Our bad sorry!",
            "Ooh Sorry, your area couldn't created on the server!"
          );
        }
      });
  }
  ...
  updateExistingEntity() {
    ...
    this.http
      .Post(updateURL, {}, { area: this.area })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "Area has been saved succesfully",
            "An area has been updated and wedding website will be impacted."
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/areas-defaults`
          );
