...
export class CategoriesFormComponent implements OnInit, AfterViewInit {
  ...
  createNewEntity() {
    ...
    this.http
      .Post(createURL, {}, { category: this.category })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "category has been saved succesfully",
            "category has been updated, Bingo!"
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/categories-defaults`
          );
        } else {
          this.ngxSpinner.hide();
          this.toastr.error(
            "Our bad sorry!",
            "Ooh Sorry, your category couldn't created on the server!"
          );
        }
      });
  }
  ...
  updateExistingEntity() {
    ...
    this.category.layout =
      this.layout == "" ? this.category.layout : this.layout;
    this.category.parentMenu =
      this.parentMenu == "" ? this.category.parentMenu : this.parentMenu;

    let updateURL = `${urls.UPDATE_CATEGORY}/${constants.APP_IDENTITY_FOR_ADMINS}/${this.category._id}`;
    this.http
      .Post(updateURL, {}, { category: this.category })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "Category has been saved succesfully",
            "A new category has been updated and wedding website will be impacted."
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/categories-defaults`
          );
  ...
  uploadImage(e: any, photoOrIcon): void {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (photoOrIcon == "photo")
        formData.append(
          "targetEntity",
          constants.S3_CONTAINERS["CATEGORIES_IMAGES"]
        );
      else
        formData.append(
          "targetEntity",
          constants.S3_CONTAINERS["CATEGORIES_ICONS"]
        );
      ...
      this.http
        ...
        .subscribe((response: responseModel) => {
          if (!response.error) {
            this.ngxSpinner.hide();
            if (photoOrIcon == "photo") this.category.photo = response.data;
            else this.category.icon = response.data;
          } else {
            this.ngxSpinner.hide();
          }
        });
