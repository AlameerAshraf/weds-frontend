...
export class CategoriesGridComponent implements OnInit, AfterViewInit {
  ...
  editEntity(id: any) {
    this.router.navigate([
      `profile/${this.lang}/admin/categories-action/update`,
    ]);
    ...
    this.http.Post(deleteURL, {}, {}).subscribe((response: responseModel) => {
      if (!response.error) {
        ...
        this.toastr.success(
          ...
          "An category has been deleted and wedding website will be impacted."
        );
        this.getAllCategories();
      } else {
        this.ngxSpinner.hide();
        this.toastr.error(
          "Our bad sorry!",
          "Ooh Sorry, your category couldn't deleted on the server!"
        );
      }
    });
  }
