...
export class OffersFormComponent implements OnInit, AfterViewInit {
  ...
  createNewEntity() {
    ...
    this.http
      .Post(createURL, {}, { offer: this.offer })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "Offer has been saved succesfully",
            "Offer has been updated, Bingo!"
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/offers-defaults`
          );
        } else {
          this.ngxSpinner.hide();
          this.toastr.error(
            "Our bad sorry!",
            "Ooh Sorry, your offer couldn't created on the server!"
          );
        }
      });
  }
  ...
  updateExistingEntity() {
    ...
    this.http
      .Post(updateURL, {}, { offer: this.offer })
      .subscribe((response: responseModel) => {
        if (!response.error) {
          this.ngxSpinner.hide();
          this.toastr.success(
            "Offer has been saved succesfully",
            "A new offer has been updated and wedding website will be impacted."
          );
          this.router.navigateByUrl(
            `/profile/${this.lang}/admin/offers-defaults`
          );
